...
<section class="orders">
   ...
   <div class="box-container">
      <?php
      $idcom=connexpdo('bookstore','myparam');
      $request3 = "SELECT * FROM `order`INNER JOIN `client` ON order.Client_Id = client.Client_Id WHERE Order_Id='$order_id'";
      ...
      if($nbart>0){
         ...
         foreach($idcom->query("SELECT * FROM `order`INNER JOIN `client` ON order.Client_Id = client.Client_Id WHERE Order_Id='$order_id' ") as $fetch_orders){
      ?>
      <div class="box">
